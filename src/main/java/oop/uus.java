...
public class uus extends Application {
    ...
    public void start(Stage peaLava) {
        ...
        //Siin tsüklites luuakse mängulaud ja mängunupud
        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 15; j++) {
                ...
                ruut.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent me) -> {
                    ...
                    if (käiguArv[0] > 8 && onVõit(ruut.getI(), ruut.getJ()))  {
                        /*
                        String v2rv = (käiguArv[0] % 2 == 0) ? "Must" : "Valge";
                        System.out.println(v2rv + " võitis mängu!");

                         */
                        //Võidu korral teatatakse, mis värv võitis
                        ...
                        try(PrintWriter kirje = new PrintWriter("gomokulog.txt", StandardCharsets.UTF_8);){
                            kirje.println("must: " + must.getText()+ ", valge: " + valge.getText() + ", võitja: " + ((käiguArv[0] % 2 != 0) ? must.getText() : valge.getText()));
    ...
    public boolean onVõit(int i, int j) {
        ...
        for (int n = 1; n < 5; n++) {
            int[] koord = {-n, 0, n};
            int pV = nuppudeAsukohad[j][i]; //preguneVärv
            String[][] ebasobivÜmbrusN = new String[3][3];
            ebasobivÜmbrusN[1][1] = "T";
            //System.out.println(Arrays.deepToString(ebasobivÜmbrusN));
            for (int xI : koord) {
                for (int xJ : koord) {
                    if (xI == 0 && xJ == 0) {
                        continue;
                    ...
                    try {
                        ...
                        if (nuppudeAsukohad[j + xJ][i + xI] != pV) {
                            ...
                            ebasobivÜmbrusN[xJ/n + 1][xI/n + 1] = "v";
                            continue;
                        }
                    } catch (EiSobiErind  e) {
                        ebasobivÜmbrusN[xJ/n + 1][xI/n + 1] = "v";
                    ...
                    ebasobivÜmbrusN[xJ/n + 1][xI/n + 1] = "t";
                }
            }
            sobivadÜmbrused[n-1] = ebasobivÜmbrusN;
        }
